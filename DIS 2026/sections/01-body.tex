...
In this work-in-progress, we explore alternative methods of communicating elusive security and privacy risks through critical designs. We present ten speculative technological creations through short vignettes that leverage humour, satire, and exaggeration to render otherwise intangible concerns perceptible and reflective. While the designs are exploratory, their grounding aligns with perspectives from usable privacy and security research that emphasize how users construct understandings of risk through informal channels, including entertainment media~\cite{fulton2019effect} and interpersonal stories~\cite{pfeffer2022replication} play a significant role in shaping users’ awareness and interpretation of privacy and security risks. By presenting plausible but exaggerated artefacts embedded in narrative contexts, critical designs function as communicative probes that externalize otherwise invisible socio-technical tensions. We argue that if users already derive security lessons from mediated narratives, then critically constructed artefacts may serve a similar pedagogical role by acting as designed media that foreground overlooked vulnerabilities and provoke reconsideration of technological assumptions. 
...
Vignettes used in research contexts, in comparison, are defined as ``short stories about hypothetical characters in specified circumstances, to whose situation the interviewee is invited to respond'' ~\cite{finch1987vignette} are established instruments in speculative design, design fiction, and scenario-based inquiry in HCI. In these contexts, narrative depictions of hypothetical technologies or practices are used to surface assumptions, value tensions, and reflections on possible futures without requiring the artefact itself to exist materially. Design fiction, in particular, is not treated as empirical stimuli for behavioural measurement, but as structured prompts that enable participants to reason about situated technological futures, their consequences, and social norms. Vignette-based methods in usable security research (e.g., ~\cite{naeini2017privacy}, ~\cite{hadan2024privacy}) similarly construct narratives to investigate user expectations, decision-making processes, and ethical considerations surrounding emerging technologies when direct observation or deployment is infeasible, unsafe, or premature.
...
We consider the first phase of this project successful in demonstrating that thought-provoking yet plausible critical designs can be produced for security and privacy contexts. In the second phase, we will refine this exploratory process into a reproducible methodology for constructing such designs. Building on insights from the first phase, we will develop structured ideation cards representing concepts, misconceptions, domains, and interaction modalities. These cards will be combined in small sets to scaffold exploration and examine how constrained prompts shape design diversity, thematic coverage, and narrative plausibility. We will also explore generative AI as an ideation partner (the designs in this paper were developed without AI assistance). To better understand these processes, we will document the design work through autoethnography to identify recurring practices and challenges, including topic selection and balancing plausibility with provocation. The resulting methodology is intended to be transferable beyond security and privacy, supporting critical design development in adjacent domains (\eg AI, algorithmic bias, deceptive patterns, \etc).
